...
recovery. A scripted multi-step workflow engine is not required.
...
job-scoped context, transcripts, artifacts, and resumable state. Arbitration
remains a manager responsibility, not a separate role.
...
after that gate; an intermediate RPC migration is not required.

Event Sorcery TypeScript bindings for job and coordination state are a future
direction, not a prerequisite. Raw session text remains session history.
...
an interface; closing it does not stop the service or its jobs. This record
