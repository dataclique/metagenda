...
test("the cursor lane is retired and builds no probe", () => {
  assert.equal(planErrorCode(cursorPayload), "invalid_input")
  assert.equal(
    planErrorCode({ ...cursorPayload, model: "composer-2.5" }),
    "invalid_input",
  )
})
