...
import { isValidRegister, getRegister, deleteToRegister } from "../registers.ts"
import type { Position } from "../motions.ts"
import {
  beginSearch,
  searchNext,
  searchPrev,
    ...
    if (reg) {
      ...
      if (reg.linewise) {
        ...
        if (pasteLines.length === 1) {
          newLines[pos.line] =
            line.substring(0, pos.col) + reg.text + line.substring(pos.col)
          ctx.setText(newLines.join("\n"))
