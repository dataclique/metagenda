...
import { resetOperatorState } from "../state.ts"
import { isDigit, ESCAPE_SEQS } from "../keys.ts"
import {
  charLeft,
  charRight,
  wordForward,
...
import { getRegister, isValidRegister } from "../registers.ts"
import {
  startRecording,
  recordKey,
  markInsertEntry,
...
import { resetReplaceState } from "./replace.ts"
import { handleInsertMode } from "./insert.ts"
import { logicalVerticalPosition } from "../cursor.ts"
import {
  beginSearch,
  searchNext,
  searchPrev,
...
function executeMotion(
  ...
  if (result.failed) {
    if (ctx.state.pendingOperator) discardRecording()
...
export function handleNormalMode(
  ...
  if (data === "p" || data === "P") {
    ...
    if (reg) {
      paste(ctx, reg.text, reg.linewise, data === "P", count)
    }
    finalizeChangeRecording(state)
    resetOperatorState(state)
    return true
  ...
  switch (data) {
    ...
    case "j":
      if (state.pendingOperator) {
        ...
        moveLogicalVertical(ctx, 1, count)
        resetOperatorState(state)
      }
      return true
    ...
    // === Basic editing ===
    case "s": {
      beginChangeRecording(state, "s", count)
      const lines = ctx.getText().split("\n")
      const cursor = ctx.getCursor()
      const line = lines[cursor.line] || ""
      if (cursor.col < line.length) {
        const range: OperatorRange = {
          start: { line: cursor.line, col: cursor.col },
            ...
            line: cursor.line,
            col: Math.min(cursor.col + count - 1, line.length - 1),
          },
          linewise: false,
          inclusive: true,
        }
        ...
        ctx.setText(result.newLines.join("\n"))
        const insertionLine = result.newLines[cursor.line] || ""
        ctx.moveCursorTo(
          cursor.line,
          Math.min(cursor.col, insertionLine.length),
        )
      }
      markInsertEntry()
      state.mode = "insert"
    ...
    case "~": {
      ...
      if (cursor.col < line.length) {
        ...
        lines[cursor.line] =
          line.substring(0, cursor.col) + toggled + line.substring(end)
        ctx.setText(lines.join("\n"))
        ctx.moveCursorTo(
          cursor.line,
          Math.min(end, Math.max(0, (lines[cursor.line] || "").length - 1)),
        )
      }
      finalizeChangeRecording(state)
      resetOperatorState(state)
...
function replayLastChange(ctx: NormalModeContext, countOverride: number): void {
  ...
  try {
    ...
    const keys =
      countOverride > 0
        ? [
            ...
            ...change.keys.slice(firstCommand < 0 ? 0 : firstCommand),
          ]
        : change.keys
    ...
    if (
      ...
      // Exit insert/replace mode through the matching mode handler so counted
      // open-line insertions apply and clear their state exactly once.
      if (wasReplace) state.mode = "normal"
      else handleInsertMode("\x1b", ctx)
    }
  } finally {
...
function paste(
  ...
  if (linewise) {
    const pasteLines = Array.from({ length: count }, () => text)
      .join("\n")
      .split("\n")
    if (before) {
    ...
    if (before) {
      ...
      if (repeatedText.includes("\n")) {
        ...
        ctx.moveCursorTo(cursor.line, cursor.col + repeatedText.length - 1)
      }
    } else {
      // p: paste after cursor
      const insertCol = Math.min(cursor.col + 1, line.length)
      const pasteLines = repeatedText.split("\n")
      ...
      if (pasteLines.length === 1) {
        ...
        lines[cursor.line] = newLine
        ctx.setText(lines.join("\n"))
        ctx.moveCursorTo(cursor.line, insertCol + repeatedText.length - 1)
      } else {
...
function moveLogicalVertical(
  ...
  const lines = ctx.getText().split("\n")
  const target = logicalVerticalPosition(
    lines,
    ctx.getCursor(),
    direction,
    count,
  )
  ctx.moveCursorTo(target.line, target.col)
}
