...
import type { MotionFn, MotionResult, Position } from "./motions.ts"
...
export function getWordUnderCursor(
  ...
  while (start > 0 && /[\p{L}\p{N}_]/u.test(line[start - 1]!)) start--

  // Expand right
  let end = cursor.col
  while (end < line.length - 1 && /[\p{L}\p{N}_]/u.test(line[end + 1]!)) end++
